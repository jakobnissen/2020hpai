...
function remove_primers(name::String, seq::NucleotideSeq, primers::Vector{<:Tuple{String, NucleotideSeq}}, minlength::Int)
    overlaps = [slide!(p, seq, minlength) for (h,p) in primers]
    if maximum(overlaps) > 0
        i = argmax(overlaps)
        overlap = overlaps[i]
        header = primers[i][1]
        println(stdout, "$name: Primer $header found with overlap $overlap")
        seq = seq[1+overlap:end]
    end
    return seq
end

function remove_primers(name::String, primerpath::String, consensuspath::String, output::String, minlength::Int)
    primers = load_primers(primerpath)
    header, seq = load_consensus(consensuspath)

    if !isempty(primers)
        seq = remove_primers(name, seq, primers, minlength)
        seq = remove_primers(name, reverse_complement(seq), primers, minlength)
...
renames = Dict(
        ...
        "14600-1-01_S11" => "A/common buzzard/Denmark/14600-1/2020-11-06(H5N8)",
        ...
        "14535-1-01_S5" => "A/graylag goose/Denmark/14535-1/2020-11-09(H5N8)",
...
for segment in SEGMENTS
    for basename in basenames
        ...
        remove_primers(basename * '_' * segment, primers, consensus, output, 7)
